package org.cst8277.GanWei.gateway.web.rest.errors;
...
public class ExceptionTranslator extends ResponseEntityExceptionHandler implements ExceptionTranslation {
    ...
    private ProblemDetailWithCause getProblemDetailWithCause(Throwable ex) {
        if (
            ex instanceof org.cst8277.GanWei.gateway.service.UsernameAlreadyUsedException
        ) return (ProblemDetailWithCause) new LoginAlreadyUsedException().getBody();
        if (
            ex instanceof org.cst8277.GanWei.gateway.service.EmailAlreadyUsedException
        ) return (ProblemDetailWithCause) new EmailAlreadyUsedException().getBody();
        if (
            ex instanceof org.cst8277.GanWei.gateway.service.InvalidPasswordException
