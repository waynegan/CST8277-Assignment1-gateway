package org.cst8277.GanWei.gateway.web.rest;
...
import org.cst8277.GanWei.gateway.config.Constants;
import org.cst8277.GanWei.gateway.domain.User;
import org.cst8277.GanWei.gateway.domain.UserFeign;
import org.cst8277.GanWei.gateway.domain.UserRole;
import org.cst8277.GanWei.gateway.repository.UserRepository;
import org.cst8277.GanWei.gateway.security.AuthoritiesConstants;
import org.cst8277.GanWei.gateway.service.MailService;
import org.cst8277.GanWei.gateway.service.UserService;
import org.cst8277.GanWei.gateway.service.dto.AdminUserDTO;
import org.cst8277.GanWei.gateway.web.rest.errors.BadRequestAlertException;
import org.cst8277.GanWei.gateway.web.rest.errors.EmailAlreadyUsedException;
import org.cst8277.GanWei.gateway.web.rest.errors.LoginAlreadyUsedException;
 ...
 * <p>
 * This class accesses the {@link org.cst8277.GanWei.gateway.domain.User}
 * entity, and needs to fetch its collection of authorities.
 * <p>
...
public class UserResource {
    ...
    public UserResource(
        ...
        this.webClient = webClientBuilder.baseUrl("http://localhost:8081/api/users/register").build();
    }
    ...
    public Mono<ResponseEntity<User>> createUser(@Valid @RequestBody AdminUserDTO userDTO) {
        ...
        return userRepository
            ...
            .flatMap(user -> {
                ...
                // Prepare userFeign object
                UserFeign userFeign = new UserFeign();
                userFeign.setUsername(user.getLogin());
                userFeign.setEmail(user.getEmail());
                userFeign.setPassword(user.getPassword());
                LOG.debug("userFeign: {}", userFeign);

                if (user.getAuthorities().size() > 1) {
                    userFeign.setRoles(UserRole.BOTH);
                } else {
                    String authorityName = user.getAuthorities().iterator().next().getName();
                    userFeign.setRoles(UserRole.valueOf(authorityName.replace("ROLE_", "")));
                }

                LOG.debug("Sending request to microservice...");
                return webClient
                    .post()
                    .bodyValue(userFeign)
                    .retrieve()
                    ...
                    .onErrorResume(WebClientResponseException.class, e -> {
                        LOG.error("Failed to register user in microservice: {}", e.getMessage());
                        return userService
                            .deleteUser(user.getLogin())
                            .then(Mono.error(new RuntimeException("Failed to register user in microservice")));
                    })
                    .doOnTerminate(() -> LOG.debug("Request to microservice completed"))
                    .thenReturn(user);
            })
            .map(user -> {
                try {
